...
static int send_signature(int sock, const char *fname) {
  ...
  do {
    if ((bufs.eof_in == 0) && (bufs.avail_in < sizeof(in_buf))) {
      ...
      /* Fill input buffer */
      size_t n_bytes = fread(in_buf + bufs.avail_in, 1,
                             sizeof(in_buf) - bufs.avail_in, file);
      if (n_bytes == 0) {
        if (ferror(file)) {
...
static int recv_delta_and_patch_file(int sock, const char *fname_old) {
  ...
  /* Open new file */
  FILE *f_new = rs_file_open(fname_new, "wb", 1);
  assert(f_new != NULL);

  // SELF-NOTE: might not be possible to use fstream for the old file
  /* Open basis file */
  FILE *f_old = rs_file_open(fname_old, "rb", 0);
  assert(f_old != NULL);

  rs_job_t *job = rs_patch_begin(rs_file_copy_cb, f_old);
  assert(job != NULL);
  ...
  do {
    ...
    if ((bufs.eof_in == 0) && (bufs.avail_in < BUFFER_SIZE)) {
      ...
      size_t n_bytes;
      int ret =
          recv_message(sock, in_buf + bufs.avail_in, &n_bytes, &bufs.eof_in);
      if (ret == -1) {
        rs_file_close(f_new);
        rs_file_close(f_old);
        rs_job_free(job);
    ...
    size_t present = (size_t)(bufs.next_out - out_buf);
    if (present > 0) {
      size_t n_bytes = fwrite(out_buf, 1, present, f_new);
      if (n_bytes == 0) {
